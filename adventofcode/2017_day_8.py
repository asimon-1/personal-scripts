...
input_list = [
    p.replace(",", "").replace("inc", "+=").replace("dec", "-=")
    for p in raw.split("\n")
]
words = [
    p.split(" ")[0] for p in input_list
]  # Assumes that every needed word is modified in the instructions at some point.
words = dict(
    zip(set(words), [0] * len(set(words)))
)  # Store word values in a dictionary.
for w in words:
    pattern = "(?<!.){}\s|\s{}\s".format(
        w, w
    )  # Look for matches at the beginning of the instruction or in the condition
    regex = re.compile(pattern)
    input_list = [
        regex.sub(" words['{}']".format(w), i) for i in input_list
    ]  # Replace the words with their dictionary equivalent
...
for cond, instr in zip(conditions, instructions):
    ...
    if eval(
        cond
    ):  # Evaluate the condition and execute the corresponding instruction if it passes
        print("Condition Passed, Executing {}".format(instr))
        exec(instr)
        max_value = max(
            max_value, max(words.values())
        )  # Keep track of the largest value of all time
print("\n\nExecution Complete.")
