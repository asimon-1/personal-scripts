...
program_list = [
    program(name, children=children[i], weight=weights[i])
    ...
    for p2 in program_list:
        if len(p2.heritage) == size:
            ...
            if p2.children == []:
                ...
                p2.supporting_weight += sum(
                    [program_dict[p3].supporting_weight for p3 in p2.children]
                )
...
for size in xrange(tree_size):
    ...
    for c in [program_dict[p] for p in current.children]:
        if c.supporting_weight != common:
            print(
                "{} supporting_weight = {} when it should have been {}".format(
                    c.name, c.supporting_weight, common
                )
            )
            print(
                "If this is the last message, the answer is {}".format(
                    c.weight - (c.supporting_weight - common)
                )
            )
            current = c
