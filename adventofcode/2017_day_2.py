...
s = sum([(np.max(row)-np.min(row)) for row in input_array.tolist()])
print(s)
...
# ])
s = 0
for i,row in enumerate(input_array.tolist()):
    # Check if n/m for
    for n in row:
        for m in row:
            if n != m and (float(n)/float(m) - np.floor(float(n)/float(m)))<0.00001:
                print('Adding {} to the total for row {}'.format(np.floor(float(n)/float(m)),i))
                s += np.floor(float(n)/float(m))
print(s)