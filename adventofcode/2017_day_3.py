...
step_list = np.concatenate((np.arange(max_steps,min_steps,-1),np.arange(min_steps,max_steps,1)))
...
center_row,center_column = int(np.ceil(size/2.)),int(np.ceil(size/2.))
direction = 'right'

container[center_row][center_column] = 1
current_row,current_column = int(np.ceil(size/2.)),int(np.ceil(size/2.))
...
for i in range(100):
    ...
    if direction == 'right':
        current_column += 1
        container[current_row][current_column] += container[current_row-1][current_column-0]
        container[current_row][current_column] += container[current_row-1][current_column-1]
        container[current_row][current_column] += container[current_row-0][current_column-1]
        container[current_row][current_column] += container[current_row+1][current_column-1]
        container[current_row][current_column] += container[current_row+1][current_column+0]
        container[current_row][current_column] += container[current_row+1][current_column+1]
        container[current_row][current_column] += container[current_row+0][current_column+1]
